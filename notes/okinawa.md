...
| 角色 | 行李 | 用途摘要 |
|------|------|---------|
| 🔴 託運 | 29 吋（子擎額度，兩人共用）| 衣物模組（雙面袋）· 璇內衣 · 一次性睡衣褲 · 盥洗袋 · 備用藥 · ✂️ 小剪刀 |
| 🟡 子擎登機 | 16 吋行李箱 | Niko 白色手提箱（GoPro＋空拍機）· 黃色充電硬盒 · TNF 腳架 · GoPro 頸掛架 |
| 🟡 佩璇手提登機 | Moshi Costa 花崗岩灰（51×30×15）| 床頭過夜包 · 充電過夜模組包 · TNF 大 Stash（⚡ 電池）· 輕便外套 |
| 🟢 子擎隨身 | Salomon 20L | Bellroy 腰包（證件・錢包）· KIU 包 · TNF 小 Stash（卡針・AirTag・收據包・卡套）|

**必記規則：**
- ✂️ **小剪刀**：不可隨身，需放 **29 吋託運行李箱**（已從 Salomon 小 Stash 移出）
- ⚡ **電池（LiPo / 行動電源）**：不可託運，必須隨身，集中於 Moshi Costa 大 Stash
...
|------|------|------------|--------|---------|------|
| 🔴 託運 | 29 吋行李箱 | — | 共用（子擎額度）| 衣物模組（黑色雙面袋）· 璇內衣 · 子擎一次性睡衣褲 · 子擎盥洗袋 · 備用牙刷牙線袋 · 備用藥袋 · ✂️ 小剪刀 | 子擎一次性內褲褲每日可丟，減輕回程重量 |
| 🟡 登機 | 16 吋行李箱 | — | 子擎 | Niko 白色手提箱（GoPro ＋ 空拍機機身）· 黃色充電硬盒 · TNF 腳架 · GoPro 頸掛架 | 設備箱；⚡ 電池取出放 Moshi Costa 大 Stash |
| 🟡 手提登機 | Moshi Costa 花崗岩灰 | 51 × 30 × 15 cm | 佩璇 | 床頭過夜包 · 充電過夜模組包 · TNF 大 Stash（⚡ 電池）· 輕便外套 | ✅ 三邊＜56×36×23，沒風險 |
| 🟢 隨身 | Salomon 20L | 20 L | 子擎 | Bellroy 腰包（兩人護照・駕照・錢包）· KIU 包 · TNF 小 Stash（卡針・AirTag・收據包・卡套）| 子擎外出主包 |
...
| 模組袋 | 內容 |
|-------|------|
| 快速衣物模組（黑色雙面袋）| 每日換洗衣物（子擎 + 佩璇共用）— 上衣 · 褲子 · 襪子 · 內褲 |
| 佩璇內衣袋 | 璇內衣 |
| 子擎一次性睡衣睡褲袋 | 一次性睡衣睡褲（每晚丟棄，減輕回程重量）|
...
| 物品 | 備註 |
|------|------|
| 備用藥透明袋 | 出國備用藥品（詳見行李備用藥對照表）|

**雜項（注意）**

| 物品 | 備註 |
|------|------|
| ✂️ 小剪刀 | ⚠️ **不可隨身，需託運** — 由 Salomon 小 Stash 移至此處 |

> ⚠️ 僅子擎有託運額度（AirAsia 20 kg / Tiger Air 25 kg），**佩璇無加購託運**，本箱為兩人共用。  
> 💡 出發建議預留 **3–5 kg 空間**，留給回程伴手禮；若超重，輕型物品可轉移至 Salomon 隨身帶。
...
| 物品 | 內容 | 備註 |
|------|------|------|
| 白色 Niko 手提箱 | GoPro 機身 · 空拍機機身 | ⚡ 電池取出，另外放 Moshi Costa 大 Stash 隨身帶 |
| 黃色充電硬盒 | 充電器 · 線材 · 轉接頭 | — |
| TNF 腳架 | 攝影腳架 | — |
| GoPro 頸掛架 | 頸掛式拍攝配件 | — |

> ⚠️ **空拍機電池（LiPo）不可放登機箱**，必須取出放 Moshi Costa 大 Stash 隨身帶。
...
| 模組袋 / 物品 | 內容 | 備註 |
|-------------|------|------|
| 床頭過夜包 | 眼鏡 · 隱形眼鏡 · 外傷藥 | 每晚床頭備用，方便拿取 |
| 充電過夜模組包 | 過夜充電器 · 線材 | 不需開行李箱，直接取包充電 |
| TNF 旅行收納大 Stash | ⚡ 空拍機電池（LiPo）· 行動電源 | 🚫 不可託運，必須隨身 |
| 輕便外套 | — | 飛機 / 冷氣場所備用，快速穿脫 |

> ✅ **尺寸確認（沒風險）**：Moshi Costa 51 × 30 × 15 cm，三邊均在泰航上限 56 × 36 × 23 cm 以內。子擎用 16吋（40.5×31×19.5）、佩璇用 Moshi Costa，兩人各佔一個登機 slot。
...
| 物品 | 內容 | 備註 |
|------|------|------|
| Bellroy 腰包 | 子擎駕照 · 佩璇駕照 · 兩人護照 · 錢包（兩人）| 最貼身，機場 / 景點防盜 |

**隨身收納**

| 物品 | 內容 | 備註 |
|------|------|------|
| KIU 包 | 折疊輕便背包 | 景點外出用，收納體積小 |
| TNF 旅行收納小 Stash | 卡針 · AirTag · 收據包 · 卡套 | ✂️ 小剪刀已移至 29 吋託運 |

> 💡 **件數控制**：登機時若地勤對件數嚴格，Bellroy 可塞入 Salomon，合併為一件隨身包。
