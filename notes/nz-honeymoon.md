# oceania-honeymoon
## Oceania Honeymoon Plan 2026

> 🗓 **旅程期間：** 2026/04/29（出發）～ 2026/05/17（抵台）
> 🛣 **路線：** 台灣 → 新加坡 → 澳洲（墨爾本）→ 紐西蘭（基督城 → 葛雷茅斯 → 西岸 → 瓦納卡 → 蒂卡波 → 但尼丁 → 皇后鎮）→ 台灣（經奧克蘭）
> ✅ **回程方式：** ZQN → AKL → TPE — 於紐西蘭北島奧克蘭機場轉機，12:40 抵達、19:30 出發

---

## 👥 旅行者資訊 Travelers

| | 姓名 | 護照號碼 |
|--|------|----------|
| 👨 | 子擎 | 368441734 |
| 👩 | 佩璇 | 351832004 |

---

## 🗺 行程地圖 Trip Map

> 🔗 [**擎璇大洋洲蜜月地圖（Google My Maps）**](https://www.google.com/maps/d/edit?mid=1YuxkR2DEYK-yQUwBHr1O0iwNjI1O80w&usp=sharing)  
> 景點 · 住宿 · 路線總覽 · 4/29 ~ 5/16

---

## ✈️ 航班總覽 Flight Overview

| 日期 | 路線 | 航空公司 | 航班 | 機型 | 出發 | 抵達 | 付款 | 備註 |
|------|------|----------|------|------|------|------|------|------|
| 4/29 | TPE → SIN | 酷航 Scoot | TR867 | B787-9 | 16:15 桃園 T1 | 20:45 樟宜 T1 | 佩璇 $9,328（易遊網）| 每人 20kg |
| 5/1 | SIN → MEL | 捷星 Jetstar | JQ008 | B787-8 | 21:00 樟宜 T2 | 06:15+1 墨爾本 T2 | 佩璇 $21,156（易遊網）| 每人 20kg；飛機上過夜 |
| 5/4 | MEL → CHC | 捷星 Jetstar | JQ171 | A320 | 11:05 墨爾本 T2 | 16:25 基督城 I 廳 | 佩璇 $15,694（trip.com）| 每人 30kg |
| 5/16 | ZQN → AKL | 捷星 Jetstar | JQ292 | A320 | 10:50 皇后鎮 D 廳 | 12:40 奧克蘭 D 廳 | 子擎 $33,864（trip.com）| 每人 30kg；含下段 AKL→TPE |
| 5/16 | AKL → TPE | 中華航空 | CI054 | A350 XWB | 19:30 奧克蘭 I 廳 | 05:40+1 桃園 | （同上，子擎合併付）| 每人 2×23kg；行李直掛 |

---

## 🧳 行李配置 Oceania Packing Plan

> 兩階段系統 · 25" 前段衣物箱 → NZ 主衣物箱 · 16" 全程設備 / 備品箱

---

### 📐 行李規格總表

| 類型 | 行李 | 尺寸 / 容量 | 使用者 | Phase 1（SIN/AUS D1–D6）| Phase 2（NZ D7–D19）| 備註 |
|------|------|------------|--------|--------------------------|---------------------|------|
| 🔴 託運 | 29 吋行李箱（奶茶色） | 70 × 47 × 35 cm | 共用 | 封存 NZ 衣物 / 保暖層 / 外套 | 封存不再穿衣物 + 採買 + 低頻物品 | CHC 住宿後角色切換 |
| 🔴 託運 | 25 吋行李箱（奶茶色） | 58 × 41 × 30 cm | 共用 | 前段衣物箱（SIN/AUS 三袋）| NZ 主衣物箱 | CHC 住宿後角色切換 |
| 🟡 登機 | 16 吋行李箱（MUJI 深藍） | 40.5 × 31 × 19.5 cm | 共用 | 設備 / 備品箱 | 設備 / 備品箱 | 全程固定，不切換 |
| 🟡 手提 | Moshi Costa（花崗岩灰） | 51 × 30 × 15 cm | 共用 | 保暖外套 / 文件快取 | 保暖外套 / 文件快取 | 全程固定 |
| 🟢 隨身 | Mystery Ranch Catalyst 18（灰） | 18 L | 佩璇 | 日用隨身包 | 健行主包 | 全程佩璇使用 |
| 🟢 隨身 | Salomon 20L | 20 L | 子擎 | 日用隨身包 | 健行主包 | 全程子擎使用 |
| 🟢 隨身 | Bellroy 腰包 | 7 L | 子擎 | 護照 · 駕照 · 錢包（兩人）| 護照 · 駕照 · 錢包（兩人）| 全程子擎配戴 |

---

### 🔄 兩階段系統架構

| 行李 | Phase 1：SIN/AUS（D1–D6）| ⟳ 轉換點（5/4 CHC 住宿後）| Phase 2：NZ（D7–D19）|
|------|--------------------------|---------------------------|---------------------|
| 29" | 封存 NZ 衣物 / 保暖層 | 取出 NZ 衣物移入 25"；放入 SIN 不可丟衣物 | 封存箱 + 採買 + 低頻物品 |
| 25" | 前段衣物箱（子擎 SIN + AUS + 佩璇袋）| 清空可丟件；接收 29" NZ 衣物 | NZ 主衣物箱 |
| 16" | 設備 / 備品箱 | 不動 | 設備 / 備品箱（不變）|
| Moshi | 保暖外套 / 文件 | 不動 | 保暖外套 / 文件（不變）|
| Mystery Ranch | 佩璇日用 | — | 佩璇健行 |
| Salomon | 子擎日用 | — | 子擎健行 |
| Bellroy | 護照 / 駕照 / 錢包（兩人）| — | 同左 |

---

### 📦 29 吋行李箱角色說明

**Phase 1（SIN/AUS）：封存 NZ 衣物系統**

| 類別 | 說明 |
|------|------|
| 子擎 NZ 衣物 | 6 模組分裝（詳見 Phase 2 清單）|
| 佩璇 NZ 衣物 | 分裝一袋（待確認）|
| 保暖層 / 外套 | 刷毛中層、保暖夾克等 NZ 用 |
| 常駐備品 | 不急用配件、備用物品、採買物品 |

> 原則：Phase 1 期間盡量保持封存，不常開。

**Phase 2（5/4 CHC 住宿後）：封存 / 回收 / 採買 / 低頻使用箱**

| 類別 | 說明 |
|------|------|
| 封存不再穿衣物 | 子擎 SIN 短袖（黑 / 綠 / 粉）×3 · 佩璇 SIN / AUS 不可丟衣物 |
| 採買物品 | NZ 羊毛製品 · SIN 伴手禮 · 回程累積物品 |
| 低頻使用物品 | 備用備品 · 不急用配件 |

---

### 🔁 25 吋行李箱角色說明（核心切換箱）

> **Phase 1 = 前段衣物箱 → Phase 2 = NZ 主衣物箱**，轉換點為 5/4 基督城住宿 Check-in 後

**Phase 1（SIN/AUS）：前段衣物箱**

| 模組 | 內容 |
|------|------|
| 子擎 🇸🇬 SIN（淺藍拉鍊袋）| 短袖 · 短褲 · 內褲 · 襪子（偏可丟）|
| 子擎 🇦🇺 AUS（深灰拉鍊袋）| 短袖 · 牛仔褲 · 襪子（偏可丟）|
| 佩璇 🇸🇬🇦🇺 SIN + AUS（一袋）| 佩璇待確認 |

**Phase 2（NZ）：NZ 主衣物箱**

*5/4 到達 CHC 住宿後，將 29" 的 NZ 衣物全部移入此箱。*

**子擎 NZ 衣物模組：**

| # | 收納袋 | 內容 | 備註 |
|---|--------|------|------|
| ① | 深灰拉鍊袋 | 短袖 ×5 · 襪子 ×5 · 長褲 ×1 | NZ 逐步丟棄 |
| ② | Kiu 大袋（保暖中層）| Rab 橄欖綠刷毛 · UA 刷毛 · 其他刷毛 | Rab 第 1 天穿後收入 |
| ③ | Kiu 中袋（短褲 / 羊毛襪）| 羊毛襪 ×4 · 短褲 ×1 | — |
| ④ | Kiu 小袋（排汗底層）| Hoka · Rab · 迪卡農底層 | — |
| ⑤ | 綠色防水袋（發熱備用）| 發熱衣 ×2 · 發熱褲 ×2 | 氣溫驟降備用 |
| ⑥ | 綠色防水袋（褲類）| 牛仔褲 ×1 · Nike 機能長褲 ×1 | Nike 機能長褲可丟 |

**佩璇 NZ 衣物（一袋）：** 待佩璇確認詳細清單

---

### 📷 16 吋行李箱（設備 / 備品箱）— 全程固定

| 類別 | 內容 |
|------|------|
| 盥洗備品 | 洗面乳 · 牙刷牙膏 · 保養品（含 >100ml 液體）|
| 鞋子 | — |
| 藥品 | 急救藥品 · 常備藥 |
| 電器 | 充電器 · 線材 · 轉接頭 · 行動電源 |
| 防水袋 | 戶外活動用 |
| GoPro | 機身 + 配件 |
| 空拍機 | 機身（⚠️ 電池禁止託運，須隨身帶）|
| 腳架 | — |

> ⚠️ **空拍機電池（LiPo）**：全程禁止放入任何託運行李，需隨身帶上飛機（放 Salomon 或 Moshi）。

---

### 🎒 背包 / 手提袋

**Moshi Costa（51 × 30 × 15 cm）— 保暖外套 / 文件快取層（全程固定）**

| 內容 | 備註 |
|------|------|
| 保暖外套 | 移動中 / 停車即取，快速穿脫 |
| 重要文件 | 護照備份 · 訂房確認單 · 保險文件 |

> ⚠️ Moshi 為手提袋，容易離身。**護照正本 + 錢包請放 Bellroy 腰包**，Moshi 只放備份文件。

---

**Mystery Ranch Catalyst 18（18L）— 佩璇全程隨身背包**

| 內容 | 備註 |
|------|------|
| 日用隨身物品 | 水壺 · 零食 · 相機 · 藥品 | 
| 健行用途 | NZ 健行主背包 |

---

**Salomon 20L — 子擎全程隨身背包**

| 內容 | 備註 |
|------|------|
| 保暖機能外套 · 毛帽 · 手套 · 墨鏡 | 戶外活動配件 |
| 空拍機電池 | ⚡ 禁止託運，必須隨身 |
| 日用隨身物品 | 水壺 · 零食等 |

---

**Bellroy 7L — 子擎全程腰包**

| 內容 | 備註 |
|------|------|
| 子擎護照 · 佩璇護照 | 兩人護照均在此 |
| 子擎駕照 · 佩璇駕照 | 兩人駕照均在此 |
| 子擎錢包 · 佩璇錢包 | 現金 + 信用卡 |

---

### ⟳ 轉換點 SOP：5/4 基督城住宿 Check-in 後

| 步驟 | 動作 |
|------|------|
| ① 開箱 | 打開 29" 行李箱 |
| ② 移衣物 | 將 29" 裡所有 NZ 衣物 / 保暖層 / 外套移至 25" |
| ③ 封存舊衣 | 子擎 SIN 短袖（黑/綠/粉）×3 + 佩璇 SIN/AUS 不可丟衣物 → 放入 29" |
| ④ 確認 25" | 確認 NZ 衣物系統完整，日用衣物可取用 |
| ⑤ 確認 29" | 確認封存完成，預留採買空間 |

---

### ⚠️ 待確認事項

| 項目 | 說明 |
|------|------|
| 佩璇 SIN/AUS 衣物袋詳細內容 | 目前只有「一袋」，需確認品項與件數 |
| 佩璇 NZ 衣物清單 | 目前無清單，待確認 |
| 子擎 NZ 6 模組是否調整 | 可視打包後確認微調數量 |
| 16" 設備箱重量 | 空拍機 + GoPro + 鞋 + 電器等，需確認是否符合 7kg 登機限制 |
| 空拍機電池 Wh | 確認是否 ≤100Wh（不限）或 100–160Wh（需申報）|
| 25" Phase 2 空間 | NZ 衣物（子擎 + 佩璇）移入後是否超重或爆箱 |
| 行李箱空重 | 29" / 25" 自重未知，影響可用載重計算 |

---

## 🚗 租車資訊 Rental Car

> 📍 [**Snap Rentals CHC 位置**](https://maps.app.goo.gl/nKur63NCnnun3hFS8)

### 📍 租車摘要

| 項目 | 內容 |
|------|------|
| 租車公司 | **Snap Rentals** |
| 車型 | **Toyota Corolla Wagon** |
| 天數 | 12 天（5/4–5/16）|
| 保險 | Simply Better Cover（0 Excess · 0 Bond · 含道路救援・輪胎・擋風玻璃・鑰匙遺失）|
| 單程費 | **NZD $0**（免收 ✨）|
| 總價 | **NZD 891.58**（≈ TWD 16,557）|
| 付款 | ✅ 已付清 |

---

### 🛬 取車流程（Christchurch Airport）

1. 清關 + 提領行李後，前往 **Domestic Terminal Door 2**
2. 填寫線上接駁申請表（通關後再填）：  
   🌐 **[snaprentals.co.nz/pickup-christchurch](https://www.snaprentals.co.nz/pickup-christchurch)**  
   填寫：姓名 · 電話號碼 · 接送人數
3. 等候約 **10–15 分鐘**，接駁車為**黑白色 Snap shuttle**
4. 車場距機場約 **5 分鐘**車程

| 項目 | 內容 |
|------|------|
| 接駁服務時間 | **08:00–17:30** |
| 門市地址 | **170 Orchard Road, Harewood, Christchurch 8051** |
| 電話 | **0800 288 699**（紐西蘭本地）|
| 非營業時間 | 需提前安排，加收 **NZD $50** |

> 📍 [**Snap Rentals CHC 位置**](https://maps.app.goo.gl/nKur63NCnnun3hFS8)

> ⚠️ **取車時段注意**：班機 16:25 抵達（I 廳），清關 + 提領行李後，抵達取車點預計約 **18:00**，已超出接駁服務截止時間（17:30）。**出發前請電話聯繫 0800 288 699**，告知預計抵達時間，確認安排方式（可能需支付非營業時間費 NZD $50）。

---

### 🔴 還車資訊（Queenstown Airport）

| 項目 | 內容 |
|------|------|
| 還車時間 | **2026/5/16（週六）09:00** |
| 門市地址 | **5 Glenda Drive, Frankton, Queenstown 9300** |
| 電話 | **0800 288 699**（紐西蘭本地）|

> ⚠️ 還車當天 JQ292 10:50 起飛，**09:00 還車 → 09:30 前進入機場**，請提前打包完畢。

---

### 📋 重要規定

- 還車時須加滿油（Full to Full）
- 僅限預訂登記之駕駛人駕駛
- 違反條款將喪失全險保障

---

## 🗺️ 整體行程 Trip Timeline

| Day | 日期 | 地點 | 重點 |
|-----|------|------|------|
| D01 | 4/29 週三 | 台灣 → 新加坡 | TR867 16:15 T1 出發 → 20:45 抵達樟宜 T1；晚：**Clarke Quay** 克拉碼頭 |
| D02 | 4/30 週四 | 新加坡 🇸🇬 | 09:00 **Chinatown 牛車水** → **99 老樹** 榴槤甜品 → 13:15 **Un-Yang-Kor-Dai** 泰式（✅ 已訂）→ 15:00 **Haji Lane** → **Golden Mile** 雞飯 → ~19:00 **FATCAT**（等聯政）→ 19:30 **Lexus Durian King**（Kovan）→ **Gardens by the Bay** 光影秀 → **Marina Bay Sands** |
| D03 | 5/01 週五 | 新加坡 → 飛機上 | 上午自由 / 聖淘沙可選（USS · S.E.A. Aquarium · Skyline Luge 擇一）→ ✈ JQ008 21:00 T2 出發 |
| D04 | 5/02 週六 | 抵達墨爾本 🇦🇺 | 06:15 抵達 MEL T2；午後 **Degraves Street** → **弗林德斯街** Flinders Street Station |
| D05 | 5/03 週日 | 墨爾本 | **Queen Victoria Market**（週日市集）→ **Hosier Lane** 塗鴉巷 → **St Kilda Beach** → **弗林德斯街** |
| **NZ D1** | **5/04 週一** | **基督城 🇳🇿** | JQ171 16:25 抵達；**Snap Rentals** 取車（約 18:00，⚠️ 超出服務時間，請先電話聯繫）；晚：**Riverside Market** → **Little High Eatery**；宿：Urbanz |
| **NZ D2** | **5/05 週二** | **阿卡羅阿** | **Akaroa Heritage Park** → **阿卡羅阿港** → **Shamarra Alpacas** 羊駝農場 → 回程：**Willowbank Wildlife Reserve**（可選）；宿：Urbanz |
| **NZ D3** | **5/06 週三** | **基督城市區** | **Botanic Gardens** → **Canterbury Museum** → **Christchurch Arts Centre** → **Cathedral Square / Re:START** → **Bridge of Remembrance** → Gondola（可選）→ Recycle Boutique（可選）→ **Cookie Time Bakery**；宿：Urbanz |
| **NZ D4** | **5/07 週四** | **亞瑟隘口 → 葛雷茅斯** | **Castle Hill / Kura Tāwhiti** → **Arthur's Pass** 山口村（午餐）→ **Shantytown Heritage Park** → **Glow Worm Dell**；宿：Global Village |
| **NZ D5** | **5/08 週五** | **西岸 → 約瑟夫冰河** | **Hokitika Gorge** → **Hokitika Beach** → **Franz Josef** 冰河村；宿：Wild Kea Lodge |
| **NZ D6** | **5/09 週六** | **Fox Glacier → Haast → 瓦納卡** | **Lake Matheson**（清晨）→ **Fox Glacier** 小鎮 → **Knights Point** 海岸觀景台 → **Blue Pools** 藍池 → **Wanaka Tree** → **Beacon Point** → **Glendhu Bay**；宿：Wanaka Bakpaka |
| **NZ D7** | **5/10 週日** | **Mt Cook → 蒂卡波** | **Lake Pukaki** → **Hooker Valley Track**（White Horse Hill）→ **Church of the Good Shepherd** → **Tekapo Springs** → **Mt John Observatory**；宿：Haka House |
| **NZ D8** | **5/11 週一** | **奧馬魯 Oamaru** | **Elephant Rocks** → **Moeraki Boulders** → **Oamaru Harbour Precinct** → **Steampunk HQ** → **Scotts Brewing Co.** → **Bushy Beach** → **Blue Penguin Colony**；宿：Oamaru Backpackers |
| **NZ D9** | **5/12 週二** | **但尼丁 Dunedin** | **Dunedin Railway Station** → **Baldwin Street** → **Octagon 廣場** → **Otago Peninsula**（Taiaroa Head）；宿：Sahara Guesthouse |
| **NZ D10** | **5/13 週三** | **蒂阿瑙 Te Anau** | Dunedin 出發 → **Te Anau 蒂阿瑙** → **Redcliff Café** 晚餐；宿：Te Anau Lakefront Backpackers |
| **NZ D11** | **5/14 週四** | **米佛峽灣 → 皇后鎮** | **Kiwi Country** 集合 → **Mirror Lakes** → **Homer Tunnel** → **Milford Sound** 米佛峽灣（遊船）→ 傍晚驅車皇后鎮；宿：Black Sheep Backpackers |
| **NZ D12** | **5/15 週五** | **皇后鎮** | **Crown Range Road** 景觀公路 → **Deer Park Heights**（⚠️ 18:00 前入場）→ **Onsen Hot Pools** → **Skyline Gondola** → **Fergburger** |
| **NZ D13** | **5/16 週六** | **還車 / 離境** | 09:00 Snap Rentals ZQN 還車 → JQ292 10:50 出發 → CI054 19:30 AKL 出發 |
| D19 | 5/17 週日 | 抵達台灣 🇹🇼 | CI054 05:40 抵達桃園 |

---

## 🇸🇬🇦🇺 新加坡 / 墨爾本 詳細行程 SIN/MEL Detail

### SIN D1 ｜ 4/29（週三）｜ 台北 出發 → 新加坡 抵達

**🚆 前往機場方案**

| 方案 | 路線 | 時間 | 說明 |
|------|------|------|------|
| ✅ 最快（首選） | 板橋高鐵 → 桃園高鐵站 → A18 機捷 → T1 | **~12:30 出發，13:10 抵達** | 高鐵約 12 分 + 機捷 A18→T1 約 20 分；總計 30–40 分 |
| 🔁 保險備案 | 台北車站 A1 機捷直達車 12:30 → T1 13:06 | **12:30 搭** | 直達不轉乘，36 分鐘；[機捷班次查詢](https://www.tymetro.com.tw/tymetro-new/tw/_pages/travel-guide/timetable-search.php) |

> ⚠️ 理想 **13:15 前** 抵達桃園第一航廈；16:15 起飛，至少 3 小時前入場

| 時間 | 地點 | 停留 | 備註 |
|------|------|------|------|
| 16:15 | ✈️ 桃園 **Terminal 1** 出發 | — | TR867 酷航 B787-9 |
| 20:45 | 🛬 **樟宜機場 Terminal 1** 抵達 | — | 入境 · 提行李 · 預計 21:30 完成通關 |
| ~21:30 | 🚇 前往市區（MRT 約 40 分） | — | **Changi** CG 線 → Tanah Merah 丹那美拉（2 站 11 分）換月台 → EW 線往 Tuas Link → **Bugis 武吉士**（8 站 17 分）→ 步行到飯店 |
| 22:00 | 🏨 **[諾維維多利亞飯店](https://maps.app.goo.gl/kCf9w7fVedviGs5N9) Check-in** | — | 87 Victoria Street；⚠️ 入住繳 SGD 100 押金 |
| 22:30–23:30 | 🌉 **Clarke Quay 克拉碼頭** | 約 1 小時 | 夜間河濱酒吧街；可沿河散步或小酌 |

---

### SIN D2 ｜ 4/30（週四）｜ 新加坡市區全日 + 榴槤之夜

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 08:45 | 🚌 飯店 → Chinatown 交通 | — | — | 步行至 **Opp Natl Lib** 公車站 → 公車 **197**（Jurong East）5 站 8 分 → 下 **Opp Sri Mariamman Tp**；共約 15 分 |
| 09:00–10:30 | 🏮 **Chinatown 牛車水** | 1.5 小時 | 必去 | 佛牙寺、牛車水廟宇、伴手禮 |
| 10:30–11:00 | 🥭 **99 老樹 榴槤甜品** | 30 分鐘 | 必去 | 從 Chinatown 步行 **10 分**；榴槤甜品 / 冰淇淋 |
| 11:00–13:00 | 🚶 步行前往午餐（邊逛邊走） | — | — | 99 老樹 → Un-Yang-Kor-Dai 步行 **20 分**（逛周邊順路走） |
| **13:15–14:45** | 🍜 **Un-Yang-Kor-Dai 泰式料理** | **1.5 小時** | **必去（已訂）** | ✅ 預訂碼 **#4W9SDP3LZSE2**；僅限無現金支付；24h 前可取消 |
| 15:00–16:30 | 🎨 **Haji Lane 哈芝巷** | 1.5 小時 | 必去 | 從 Un-Yang-Kor-Dai 步行 **30 分**（邊走邊逛）；壁畫街區 · 獨立選品店 |
| 16:30–17:30 | 🍚 **Golden Mile Thien Kee 雞飯** | 1 小時 | 必去 | 從 Haji Lane 步行 **10 分**；Beach Road 老店海南雞飯 |
| 17:30–18:15 | 🚇 Golden Mile → Kovan 高文 交通 | — | — | 尼誥大道 CC MRT → **Serangoon 實龍崗**（8 站 17 分）→ 換月台 NE 線 → **Kovan 高文**（1 站 2 分）→ 步行 350m 約 5 分；共約 **45 分** |
| ~19:00 | 🍦 **[FATCAT Ice Cream & Coffee Boutique](https://maps.app.goo.gl/qnEtwgEAPfwU9ELC6)** | — | — | Kovan；等聯政到來集合點 |
| 19:30–20:00 | 🍑 **Lexus Durian King** | 30 分鐘 | 必去 | Kovan 站；與聯政一起品嘗 |
| **~20:00** | 🚕 **叫 Grab 計程車前往 Gardens by the Bay** | — | ⚠️ 務必 | 計程車約 **20 分**可達；MRT 需 50 分**趕不上**光影秀 |
| 20:30–21:15 | 🌳 **Gardens by the Bay 濱海灣花園** | 45 分鐘 | 必去 | Supertree 光影秀 **20:45** |
| 21:15–22:00 | 🛍 **Marina Bay Sands（The Shoppes）** | 1 小時 | 必去 | 夜間購物；緊鄰 Gardens |

> 📍 **原始行程有寫，但地圖未列出**：SkyPark 空中花園觀景台（可視行程安排彈性加入 D3 上午）

---

### SIN D3 ｜ 5/01（週五）｜ 新加坡 → 飛往墨爾本

| 時間 | 地點 | 停留 | 備註 |
|------|------|------|------|
| **09:30** | 🚌 飯店出發（**行李寄放飯店**） | — | 步行 2 分（130m）→ **Opp Natl Lib** → 公車 **145**（Nth Buona Vista Rd）14 站 20 分 → **HarbourFront/VivoCity** → 步行 3 分 → **Sentosa Express** 1 站 3 分 → **Resorts World 站** → 步行 2 分（190m）→ USS；共約 **45 分** |
| ~10:15–16:00 | 🎢 **Universal Studios Singapore**（可選）| ~5.5 小時 | 聖淘沙；開園 10:00；預計 16:00 前離開以趕上機場 |
| 10:15–16:00 | 🐠 **S.E.A. Aquarium**（可選）| 2–3 小時 | 聖淘沙；與 USS 擇一 |
| 10:15–16:00 | 🛷 **Skyline Luge Sentosa**（可選）| 1–2 小時 | 聖淘沙；可與海洋館搭配 |
| ~17:00 | 🏨 返回飯店取行李 | — | 原路返回約 45 分；**行李原寄放飯店** |
| ~17:05 | 🚌 飯店 → 機場 | — | 飯店出發約 **50 分**抵達 T2 |
| **18:00** | ✈️ **抵達 Terminal 2 - International** | — | 21:00 起飛，提前 3 小時入場 |
| **21:00** | ✈️ **JQ008 SIN T2 出發 → MEL** | — | 捷星 B787-8；飛機上過夜 |

---

### MEL D1 ｜ 5/02（週六）｜ 抵達墨爾本

| 時間 | 地點 | 停留 | 備註 |
|------|------|------|------|
| 06:15 | 🛬 **墨爾本 Terminal 2** 抵達 | — | 入境通關；預計 **07:00** 完成 |
| ~07:00 | 🚌 機場 → 維多利亞酒店 交通 | — | 步行 3 分（120m）→ **Melbourne Airport T3 Skybus** → **Skybus Melbourne City Express** 2 站 32 分 → Skybus Coach Terminal/Spencer St → 步行 3 分 → **電車 96**（East Brunswick）4 站 6 分 → Swanston St/Bourke St → 步行 2 分（140m）→ 酒店；共約 **1 小時** |
| **~08:00** | 🏨 **[維多利亞酒店](https://maps.app.goo.gl/MbMNZwUnDZYS3MqUA) 抵達 / 行李寄放** | — | 215 Little Collins St；check-in 15:00（可能早辦）；寄放行李後補眠 |
| 15:30–17:00 | ☕ **Degraves Street** | 1.5 小時 | 墨爾本咖啡文化窄巷；必喝精品咖啡 |
| 17:00–18:30 | 🚉 **弗林德斯街 Flinders Street Station** | 1–1.5 小時 | 黃色穹頂地標；Federation Square 步行即達 |

---

### MEL D2 ｜ 5/03（週日）｜ 墨爾本

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 09:00–11:30 | 🛒 **Queen Victoria Market** | 2.5 小時 | 必去 | 週日農夫市集；熟食攤位豐富 |
| 12:00–13:00 | 🎨 **Hosier Lane 塗鴉巷** | 1 小時 | 必去 | 緊鄰 Flinders Street；街頭藝術巷弄 |
| 13:30–15:30 | 🏖 **St Kilda Beach** | 2 小時 | 必去 | 搭 96/16 號電車約 30 分；海灘散步 + 咖啡廳 |
| 16:30–17:30 | 🚉 **弗林德斯街**（傍晚收尾）| 1 小時 | 可選 | 傍晚最美燈光；Federation Square 晚餐選擇多 |

> ⚠️ **明日行程提醒**：5/04 JQ171 11:05 出發，建議 09:00 前抵達機場（check-out 需在 11:00 前）

---

## 🗺️ 紐西蘭自駕詳細行程 NZ D1–D13

### NZ D1 ｜ 5/04（週一）｜ 基督城抵達 + 取車

| 時間 | 地點 | 停留 | 備註 |
|------|------|------|------|
| 11:05 | ✈️ MEL T2 出發（JQ171）| — | 捷星 A320 |
| 16:25 | 🛬 **基督城 I 廳** 抵達 | — | 清關、提行李 |
| 17:30–18:30 | 🚗 **Snap Rentals 取車**｜租車點 | — | 170 Orchard Road, Harewood；⚠️ 預計 18:00 抵達，已超出服務時間（截止 17:30）— **出發前電話聯繫 0800 288 699 確認安排**；接駁：[snaprentals.co.nz/pickup-christchurch](https://www.snaprentals.co.nz/pickup-christchurch) |
| 19:00–20:00 | 🛒 **Riverside Market**｜晚餐 | 1 小時 | 市中心；各國料理 |
| 20:00–21:00 | 🍽 **Little High Eatery**｜晚餐備案 | 1 小時 | 美食廣場；有機 + 手工啤酒 |
| 21:30 | 🏨 **Urbanz Check-in**｜住宿 | — | 273 Manchester Street, CHC |

**可選活動：**
- 🚡 電車晚餐 Tramway Restaurant — NZD 149（需提前預訂）
- 🍪 Cookie Time Bakery（基督城發源地，若抵達尚早可買）

---

### NZ D2 ｜ 5/05（週二）｜ 阿卡羅阿一日遊 Akaroa

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 08:30 | 🚗 出發 CHC → Akaroa | — | 82km，約 1 小時 20 分 |
| 10:00–11:30 | 🏘 **Akaroa Heritage Park** | 1–1.5 小時 | 必去 | 法式殖民歷史街區 |
| 11:30–12:30 | 🌊 **阿卡羅阿港 Akaroa Harbour** | 1 小時 | 必去 | 港口景觀 + 咖啡廳 |
| 12:30–13:30 | 🥐 午餐：Ma France 法式麵包 / 港口魚薯 | 1 小時 | — | |
| 14:00–15:00 | 🦙 **Shamarra Alpacas 羊駝農場** | 1 小時 | 必去 | 農場體驗；距 Akaroa 約 15 分車程 |
| 15:30–17:00 | 🚗 返程 Akaroa → CHC | — | 1 小時 20 分 |
| 17:00–18:00 | 🦔 **Willowbank Wildlife Reserve**（可選）| 1 小時 | 可選 | CHC 北邊（近機場），途中停留；NZD 36.5；可看奇異鳥 |
| 🏨 | **Urbanz 住宿**｜住宿 | — | |

**可選活動：**
- 🐬 阿卡羅阿自然之旅（賞海豚）— 10:45 或 13:30，NZD 125（可替換羊駝農場）

> 📍 **原始行程有寫，但地圖未列出**：阿卡羅阿來回長日遊（NZD 70，08:45–16:00 包車遊）

---

### NZ D3 ｜ 5/06（週三）｜ 基督城市區

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 08:30–10:00 | 🌿 **Botanic Gardens 植物園** | 1.5 小時 | 必去 | 免費；花卉溫室 + 玫瑰園 |
| 10:00–11:30 | 🏛 **Canterbury Museum** | 1.5 小時 | 必去 | 免費；毛利文化 + 南極展；植物園隔壁 |
| 11:30–12:15 | 🎨 **Christchurch Arts Centre** | 45 分鐘 | 必去 | 週末市集 / 工藝品；Botanic Gardens 旁 |
| 12:15–13:15 | 🍽 午餐：**Riverside Market** / **Curator's House**（可選）| 1 小時 | — | Curator's House 在植物園旁，精緻午茶 NZD 35–50 |
| 13:15–13:45 | 🏛 **Cathedral Square / Re:START** | 30 分鐘 | 必去 | 大教堂廣場 + 貨櫃商店街 |
| 13:45–14:10 | 🌉 **Bridge of Remembrance** | 20 分鐘 | 必去 | 鐘樓拱橋，一戰紀念 |
| 14:30–16:00 | 🚡 **Christchurch Gondola**（可選）| 1.5 小時 | 可選 | CHC 東南，需開車；NZD 35–40；俯瞰城市 + 坎特伯里平原 |
| 16:00–16:45 | 🛍 **Recycle Boutique**（可選）| 45 分鐘 | 可選 | Colombo St；NZ 最大二手連鎖 |
| 17:00–17:30 | 🍪 **Cookie Time Bakery Shop** | 30 分鐘 | 必買 | 基督城發源地；回台必買伴手禮 |
| 🏨 | **Urbanz 住宿**｜住宿 | — | Check-out 5/7 10:00 前 |

> 📍 **原始行程有寫，但地圖未列出**：Avon River 平底船撐篙（約 NZD 40，1 小時）；Christchurch Tram 電車一日券（NZD 40）；Canterbury Hot Air Balloon（NZD 395，06:00 出發）

---

### NZ D4 ｜ 5/07（週四）｜ 亞瑟隘口 → 葛雷茅斯

路線：CHC → Castle Hill → Arthur's Pass → Greymouth（約 250km，3 小時）

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 08:00 | 🚗 Check-out + 出發 | — | — | 建議 08:00–09:00 早出發；Urbanz Check-out 10:00 前 |
| 09:30–10:00 | 🪨 **Castle Hill / Kura Tāwhiti** | 30 分鐘 | 必去 | SH73 路側；石灰岩怪石群，免費 |
| 11:00–12:00 | 🏔 **Arthur's Pass 山口村** | 1 小時 | 必去 | 午餐：Wobbly Kea Café（山景座位）；穿越南阿爾卑斯山脈 |
| 14:00–16:30 | 🏚 **Shantytown Heritage Park** | 2–3 小時 | 必去 | 淘金時代遺跡；蒸汽火車；換裝拍照；Greymouth 南 15km |
| 19:00–19:30 | 💫 **Glow Worm Dell** | 30 分鐘 | 必去 | Greymouth 市區步行可達；夜間螢火蟲洞 |
| 🏨 | **Global Village Backpackers**｜住宿 | — | 42 Cowper Street, Greymouth |

> ⚠️ **行車注意**：出發前查詢「Arthur's Pass weather」（MetService）；積雪時改走 SH7（Canterbury Plains 替代路線）

> 📍 **原始行程有寫，但地圖誤列此處（地理上應在 D5/D6）**：
> - **Hokitika Gorge**：位於 Hokitika 南 35km 內陸，在 D5（南下 Franz Josef 途中）安排更合理 → **已移至 NZ D5**
> - **Hokitika Beach**：位於 Hokitika 市區旁，同上理由 → **已移至 NZ D5**
> - **Knights Point 海岸觀景台**：位於 Fox Glacier 以南、Haast 附近的 SH6 上，距 Greymouth 超過 200km，D4 根本無法前往 → **已移至 NZ D6**（Fox Glacier 南下途中）

---

### NZ D5 ｜ 5/08（週五）｜ 葛雷茅斯 → 西岸 → 約瑟夫冰河

路線：Greymouth → Hokitika → Franz Josef（約 175km，SH6 海岸線）

💡 **D5–D7 節奏建議**：每日選 1–2 個重點即可；此段以風景自駕為主，保留彈性時間於 Wanaka 放鬆，避免疲勞駕駛

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 08:00 | 🚗 出發 Greymouth | — | — | Check-out |
| 09:00–10:30 | 💚 **Hokitika Gorge 翡翠峽谷** | 1–1.5 小時 | 必去 | ⚠️ 地圖原列在 D4，地理上在此段（Hokitika 以南 35km 內陸）；碧藍翡翠色峽谷 |
| 10:30–11:00 | 🏖 **Hokitika Beach** | 30 分鐘 | 必去 | ⚠️ 地圖原列在 D4，同上調整；翡翠（Pounamu）文化 + 彩虹字樣海灘 |
| 11:00–14:00 | 🚗 沿 SH6 南下 | — | — | 太平洋在左，熱帶雨林在右 |
| 14:00–16:00 | 🧊 **Franz Josef 冰河村** | 2 小時 | 必去 | 輕鬆散步村落；可走冰河步道入口 |
| 🏨 | **Wild Kea Lodge**｜住宿 | — | 9 Cron Street, Franz Josef |

**已確認活動（NZ D6 早上）：**
> ✅ **法蘭士約瑟夫冰川直升機健行（Heli Hike）已訂 — D6 07:30 出發**
> Klook · 訂單 HAU427597 · 憑證 KLK0703585975 · 冰川半日遊 · 2 大人
> 地址：63 Cron Street（距 Wild Kea Lodge 步行 2 分鐘）
> ⚠️ 出發當天早上請電話確認：0800 484 337 或 +64 3 752 0763；15 分鐘前（07:15）報到
> 費用包含：靴子 · 冰爪 · 防水外套褲 · 直升機來回 · 導遊 · 登山杖 · 保暖帽手套 · 襪子 · 登山靴

---

### NZ D6 ｜ 5/09（週六）｜ 冰川直升機健行 → Fox Glacier → Haast Pass → 瓦納卡 Wanaka

路線：Franz Josef → Fox Glacier → Knights Point → Haast → Blue Pools → Wanaka（約 200km）

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 07:00 | 🏃 前往報到 | — | **✅ 已訂** | 步行至 Franz Josef Glacier Guides（63 Cron St，距 Wild Kea Lodge 步行 2 分鐘）；**請 07:15 前抵達** |
| **07:30–~10:30** | 🚁 **法蘭士約瑟夫冰川直升機健行** | **3 小時** | **✅ 已訂** | Klook HAU427597 · 冰川半日遊 · 含靴子 / 冰爪 / 防水外套 / 直升機 / 導遊 · ⚠️ 出發前電話確認：0800 484 337 |
| ~11:00 | 🚗 驅車前往 Fox Glacier | — | — | 約 25km，30 分鐘 |
| 11:30–13:00 | 🌊 **Lake Matheson 鏡湖** | 1.5 小時 | 必去 | 映照 Mt Cook + Mt Tasman；環湖步道；Fox Glacier 小鎮附近 4km（⚠️ 清晨能見度最佳，此時為上午，仍壯觀） |
| 13:00–13:30 | 🧊 **Fox Glacier 小鎮** | 30 分鐘 | 必去 | 散策；比 Franz Josef 更安靜 |
| 14:00 | 🚗 出發 SH6 南下 | — | — | |
| 15:30–15:50 | 🌊 **Knights Point 海岸觀景台** | 20 分鐘 | 必去 | ⚠️ 地圖原誤列在 D4，地理上在此段（Fox Glacier 以南約 80km SH6）；海岸礁岩全景 |
| 17:00–18:30 | 🔵 **Blue Pools 藍池**（Makarora）| 1–1.5 小時 | 必去 | 停車後步行 ~20 分（來回 45–60 分）；碧藍冰河水 |
| ~20:00 | 🌳 **Wanaka Tree / Beacon Point / Glendhu Bay** | 視狀況 | 視體力 | 傍晚抵達；直接回住宿或順路快拍 |
| 🏨 | **Wānaka Bakpaka Hostel**｜住宿 | — | 117 Lakeside Road, Wanaka |

> 📍 **原始行程有寫，但地圖未列出**：Haast Pass 瀑布群（沿 SH6 沿途停車即可，無需額外安排）

---

### NZ D7 ｜ 5/10（週日）｜ Wanaka 出發 → 庫克山 Mt Cook → 蒂卡波 Tekapo

路線：Wanaka → Lake Pukaki → Mt Cook → Tekapo（約 230km）

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 08:00 | Check-out Wanaka Bakpaka | — | — | |
| 08:30 | 🚗 出發 Wanaka → Lake Pukaki | — | — | Albert Town 車上欣賞即可 |
| 09:30–09:50 | 💧 **Lake Pukaki 普卡基湖** | 20 分鐘 | 必去 | 土耳其藍湖水；停車拍照即可 |
| 11:30–12:30 | 🍴 午餐：**Old Mountaineers Café**（Mt Cook Village）| 1 小時 | — | 冰河景觀座位；必打卡 |
| 12:30–15:30 | 🥾 **Hooker Valley Track**（White Horse Hill）| 2–3 小時 | 必去 | 停車：[White Horse Hill Campground](https://maps.app.goo.gl/FXRcJCRqedqMaE5D8)；冰河湖 + Mt Cook 正面 + 吊橋 |
| 16:00 | 🚗 離開 Mt Cook | — | — | |
| 18:30–19:15 | ⛪ **Church of the Good Shepherd** | 45 分鐘 | 必去 | 善牧教堂；Tekapo 湖畔；黃昏時段最美 |
| 19:30–21:00 | 🛁 **Tekapo Springs 湖畔溫泉** | 1.5 小時 | 必去 | 星空泡湯；建議先預訂 |
| 20:30–21:45 | 🌌 **Alpine Stargazing Experience（Silver River）** | ~75 分鐘 | **✅ 已訂** | 訂單 XYSA26X5711441；12 Rapuwai Lane；NZD 190；20:15 前抵達；天氣取消全額退款 |
| 🏨 | **Haka House Lake Tekapo**｜住宿 | — | 5 Motuariki Lane, Tekapo |

> 💡 **觀星提醒**：星空受天氣影響很大；若當晚多雲，以教堂黃昏 + 溫泉為主即可，不需強求

---

### NZ D8 ｜ 5/11（週一）｜ 奧馬魯 Oamaru

路線：Tekapo → Elephant Rocks → Moeraki Boulders → Oamaru（約 180km）

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 09:00 | 🚗 出發 Tekapo | — | — | |
| 10:00–10:20 | 🪨 **Elephant Rocks 象石**（Duntroon）| 20 分鐘 | 必去 | 石灰岩群；《納尼亞傳奇》取景地；免費 |
| 11:30–12:00 | 🌊 **Moeraki Boulders 摩拉基巨石** | 30 分鐘 | 必去 | 海灘圓石；NZD 5；落潮時最佳 |
| 13:00–14:30 | 🏛 **Oamaru Harbour Historic Precinct** | 1.5 小時 | 必去 | 維多利亞石灰岩建築街區 |
| 14:30–15:15 | 🤖 **Steampunk HQ** | 45 分鐘 | 必去 | 蒸汽龐克文化博物館 |
| 15:15–16:15 | 🍺 午餐：**Scotts Brewing Co.** | 1 小時 | 必去 | Harbour Street；精釀啤酒 + 餐點 |
| 17:00–17:45 | 🐦 **Bushy Beach** | 45 分鐘 | 可選 | 黃眼企鵝棲息地；黃昏前抵達；免費；禁止靠近 |
| 18:30–20:00 | 🐧 **Blue Penguin Colony** | 1.5 小時 | 可選 | 藍眼企鵝保護區；傍晚點名；建議提前訂位 |
| 🏨 | **Oamaru Backpackers**｜住宿 | — | ⚠️ 入住時繳 NZD 100 押金 |

> 💡 **企鵝提醒**：出現時間依季節；白天先安排 Harbour 區，傍晚留給企鵝觀察

---

### NZ D9 ｜ 5/12（週二）｜ 但尼丁 Dunedin

路線：Oamaru → Dunedin（約 120km，1.5 小時）

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 09:00 | 🚗 出發 Oamaru | — | — | |
| 10:30–11:00 | 🚉 **Dunedin Railway Station** | 30 分鐘 | 必去 | 蘇格蘭巴洛克建築 |
| 11:00–11:20 | 📐 **Baldwin Street** | 20 分鐘 | 必去 | 世界最陡住宅街道之一 |
| 11:20–12:15 | 🏙 **Octagon 市中心廣場** | 45 分鐘 | 必去 | |
| 12:15–13:15 | 🍽 午餐：Nova Café（Stuart Street）| 1 小時 | — | 人氣老牌咖啡館 |
| 14:00–17:00 | 🦭 **Otago Peninsula（Taiaroa Head）** | 2–3 小時 | 可選 | 企鵝 + 皇家信天翁；視時間與體力安排 |
| 🏨 | **Sahara Guesthouse & Motels**｜住宿 | — | |

---

### NZ D10 ｜ 5/13（週三）｜ 但尼丁 → 蒂阿瑙 Te Anau

路線：Dunedin → Te Anau（約 4–4.5 小時）

| 時間 | 地點 | 停留 | 備註 |
|------|------|------|------|
| 08:00 | 🚗 出發 Dunedin | — | 建議早出發 |
| 途中 | 沿途自備午餐 | — | 建議 Dunedin 超市購買 |
| 約 13:00 | 🌿 **Te Anau 蒂阿瑙** 抵達 | — | |
| 14:00–18:00 | 🌊 蒂阿瑙湖畔散步 / 休息 | 約 4 小時 | Milford Sound 前一晚；建議早入住早休息 |
| 19:00–20:30 | 🍽 **Redcliff Café** 晚餐 | 1.5 小時 | 蒂阿瑙特色餐廳；建議訂位 |
| 🏨 | **Te Anau Lakefront Backpackers**｜住宿 | — | Check-in 14:00–20:00；花園艙間 |

---

### NZ D11 ｜ 5/14（週四）｜ 米佛峽灣大巴一日遊 → 皇后鎮

**GreatSights 米佛峽灣大巴一日遊（✅ 已訂，Klook NT$6,730 · 2 位成人）**

| 時間 | 地點 | 停留 | 備註 |
|------|------|------|------|
| 09:30–09:45 | 📍 **Kiwi Country**｜集合點 | — | Miro 街 2 號，Te Anau；[地圖](https://maps.app.goo.gl/NF3ZGAi5TVs63enRA) |
| **09:45** | 🚌 **大巴出發** | — | GreatSights 米佛峽灣一日遊 |
| 途中 | 🌊 **Mirror Lakes 鏡湖** | 約 20 分鐘 | 巴士途中停留 |
| 途中 | 🚇 **Homer Tunnel 荷馬隧道** | 約 10 分鐘 | 通行 |
| **12:50–15:00** | ⛵ **Milford Sound 米佛峽灣** | 1.5–2 小時 | 登船遊覽；Stirling / Bowen 瀑布；千仞峭壁 |
| **17:15** | 🚌 回程出發（Te Anau 約 19:30）| — | |
| **21:00–22:00** | 🚗 驅車皇后鎮（約 1.5–2 小時）| — | |
| 🏨 | **Black Sheep Backpackers**｜住宿 | — | Check-in 15:00–23:30 |

> ⚠️ 出發前 24 小時致電 **0800 866 666** 確認接載地點與時間
> 免費取消至 5/13（三）00:00 NZST

---

### NZ D12 ｜ 5/15（週五）｜ 皇后鎮自由活動

| 時間 | 地點 | 停留 | 必去 / 可選 | 備註 |
|------|------|------|------------|------|
| 09:00–11:30 | 🛣 **Crown Range Road 瞭望點** | 2–2.5 小時 | 可選 | 紐西蘭最高公路；往 Wanaka 方向；景觀壯闊；⚠️ 冬季留意路況 |
| 12:00 | 返回皇后鎮市區 | — | — | 午餐自理 |
| **13:00–17:00** | 🦌 **Deer Park Heights** | 2–4 小時 | **必去（已訂）** | ⚠️ **18:00 前須入場（閘門關閉）**；NZD $75/車；僅限自駕；Hard Antler Block 全程留車內 |
| 17:30–19:00 | ♨️ **Onsen Hot Pools 溫泉** | 1.5 小時 | 必去 | 放鬆泡湯；建議訂位 |
| 19:00–20:30 | 🚠 **Skyline Gondola 天際線纜車** | 1.5 小時 | 可選 | 俯瞰 Lake Wakatipu；景觀餐廳 Stratosfare |
| **21:00–22:00** | 🍔 **Fergburger** | 1 小時 | 必去 | 21:00 後排隊較短 |
| — | 🧳 整理行李 | — | — | ⚠️ 明早 09:00 Snap Rentals ZQN 還車；加滿油 |

> 💡 **順序調整說明**：地圖順序將 Crown Range Road 列在 Deer Park Heights 之前。Crown Range Road 晨間景觀最佳，Deer Park Heights 午後安排（確保 18:00 前入場），調整後符合地圖順序且時間合理。

> 📍 **原始行程有寫，但地圖未列出**：跳傘 / 噴射船 / 高空彈跳（可選加入 Deer Park 前）

---

### NZ D13 ｜ 5/16（週六）｜ 還車 / 離境

| 時間 | 地點 | 備註 |
|------|------|------|
| 07:00 | 起床準備 | |
| **09:00** | 🚗 **Snap Rentals ZQN 還車** | 5 Glenda Drive, Frankton, QTN 9300；⚠️ 加滿油（Full to Full）|
| **09:30 前** | 🛂 機場報到 | ZQN Airport；建議 09:30 前進入 |
| **10:50** | ✈️ JQ292 ZQN → AKL | 12:40 抵達 AKL D 廳 |
| 12:40–19:30 | 🏙 奧克蘭轉機（~7 小時）| D 廳 → I 廳；可出市區或機場附近逛街；行李直掛桃園 |
| **19:30** | ✈️ CI054 AKL → TPE | 中華航空 A350 XWB；途經布里斯本；飛機上過夜 |
| 05:40+1 | 🏠 抵達桃園 | 5/17（週日）|

---

## 🏨 住宿規劃 Accommodation

### 新加坡 Singapore（4/29 ~ 5/01，2 晚）✅ 已訂

**諾維維多利亞飯店（Victoria Hotel, NuVe Group Collection）**  
87 Victoria Street, Singapore 188016

| 項目 | 內容 |
|------|------|
| Check-in | 4/29（三）15:00 |
| Check-out | 5/01（五）11:00 |
| 房型 | 高級特大床房 |
| 付款 | Trip.com Apple Pay — 4/11 子擎已付 **TWD $5,156**（不退款）|
| Hotel 確認碼 | 1616329485771904 |
| 訂單號 | 1616329485770843 |
| 入住憑證 | [新加坡入住憑證.pdf](../docs/assets/新加坡入住憑證.pdf) |
| 注意事項 | 入住時須繳交 **SGD 100** 押金；不退款 |
| TEL | +65-65506088 |

### 墨爾本 Melbourne（5/02 ~ 5/04，2 晚）✅ 已訂

**維多利亞酒店（The Victoria Hotel）**  
215 Little Collins Street, Melbourne VIC 3000

| 項目 | 內容 |
|------|------|
| Check-in | 5/02（六）15:00 |
| Check-out | 5/04（一）11:00 |
| booking.com 確認碼 | **6679485633** |
| PIN | 7004 |
| 付款 | 子擎 中信 Line Pay 末碼 0950，4/29 排程扣款 **AUD $322.03**（≈ TWD $7,239）|
| 免費取消截止 | **4/30 23:59 AEST** |

### 基督城 Christchurch（5/04 ~ 5/06，3 晚）✅ 已訂

**Urbanz 烏爾班茲旅舍**  
273 Manchester Street, Christchurch

| 項目 | 內容 |
|------|------|
| Check-in | 5/04（一）|
| Check-out | 5/07（四）6:00–10:00 前 |
| 晚數 | 3 晚（NZ D1–D3）|
| TEL | +64 3 366 4414 |
| 付款 | 子擎 Line Pay 中信信用卡 · 4/13 訂房 · 4/30 排程扣款 **NZD 345.65**（≈ TWD 6,418）|
| 免費取消截止 | **5/02** |

📌 入住注意事項
- 🚗 停車：免費路邊停車（不可預約），旁邊另有付費停車場
- 🧺 洗衣：NZD 12 / 次（洗 + 烘 + 送回房間，代洗服務）
- ⚠️ 無自助洗衣設備

### 葛雷茅斯 Greymouth（5/07，1 晚）✅ 已訂

**地球村旅客酒店 Global Village Travellers Lodge**  
42 Cowper Street, Greymouth

| 項目 | 內容 |
|------|------|
| Check-in | 5/07（四）|
| Check-out | 5/08（五）|
| 付款 | 子擎 Line Pay 中信信用卡 · 4/13 訂房 · 5/03 排程扣款 **NZD 99**（≈ TWD 1,838）|
| 免費取消截止 | **5/05** |

📌 入住注意事項
- 🚗 停車：免費（不需預約，車道左側或門口）
- 🚂 接送：可提前 email 預約火車站接送

### 約瑟夫冰河 Franz Josef（5/08，1 晚）✅ 已訂

**Wild Kea Lodge Franz Josef**  
9 Cron Street, Franz Josef

| 項目 | 內容 |
|------|------|
| Check-in | 5/08（五）|
| Check-out | 5/09（六）|
| 付款 | 子擎 Line Pay 中信信用卡 · 4/13 訂房 · 5/03 排程扣款 **NZD 112.50**（≈ TWD 2,089）|
| 免費取消截止 | **5/05** |

📌 入住注意事項
- 🕒 Check-in：14:00 ｜ Check-out：10:00
- 🏢 冬季櫃檯時間：08:30–18:00
- 🔑 Late check-in：18:00 後需提前電話通知；lockbox 備用（密碼：1234，內有房卡）
- 🚪 早退：直接投鑰匙即可
- 📞 電話：027 368 7695
- ⚠️ 出發前確認天氣與路況（冰河地區天候多變）

### 瓦納卡 Wanaka（5/09，1 晚）⭐ ✅ 已訂

**Wanaka Bakpaka**  
117 Lakeside Road, Wanaka

| 項目 | 內容 |
|------|------|
| Check-in | 5/09（六）|
| Check-out | 5/10（日）|
| Hostelworld 訂單 | #286598-576809353 |
| 付款 | 子擎 · 4/13 訂房 · Hostelworld **€10.30** 已付 + 到店付 **NZD 110.50**（合計 ≈ TWD 2,414）|
| 免費取消截止 | **5/06** |

### 蒂卡波 Lake Tekapo（5/10，1 晚）⭐⭐ ✅ 已訂

**Haka House Hostel Tekapo**  
5 Motuariki Lane, Tekapo

| 項目 | 內容 |
|------|------|
| Check-in | 5/10（日）|
| Check-out | 5/11（一）|
| booking.com 確認碼 | **6182737200** |
| PIN | 5390 |
| 付款 | 子擎 Line Pay 中信信用卡 · 4/13 訂房 · **NZD 337.36 已付**（4/14，≈ TWD 6,264）|
| 取消政策 | **不退款** |

### 奧馬魯 Oamaru（5/11，1 晚）✅ 已訂

**奧馬魯背包客旅館（Oamaru Backpackers）**

| 項目 | 內容 |
|------|------|
| Check-in | 5/11（一）|
| Check-out | 5/12（二）|
| booking.com 確認碼 | **5970115635** |
| PIN | 5837 |
| 付款 | 子擎 Line Pay 中信信用卡 · 4/13 訂房 · 5/06 排程扣款 **NZD 139.32**（≈ TWD 2,587）|
| 免費取消截止 | **5/08** |
| 注意事項 | 入住時須繳交 **NZD 100** 押金 |

📌 入住注意事項
- 🔑 自助 Check-in（Goki 系統）：付款後收 email（含 PIN + 房號）
- ⏳ PIN 需付款才啟用；若 15 分鐘未收到需聯絡客服
- 🚗 停車：免費（數量有限，先到先停）
- ⛔ 嚴禁攜帶外人入住（罰款 NZD 500 / 人 / 晚）
- ⚠️ 請確認床位正確，使用錯誤床位收 NZD 100 / 天清潔費

### 但尼丁 Dunedin（5/12，1 晚）✅ 已訂

**Sahara Guest House**

| 項目 | 內容 |
|------|------|
| Check-in | 5/12（二）|
| Check-out | 5/13（三）|
| 付款 | 子擎 Line Pay 中信信用卡 · 4/13 訂房（≈ TWD 2,724）|
| 免費取消 | ✅（截止日期待補，確認後可視需要保留）|

### 蒂阿瑙 Te Anau（5/13，1 晚）✅ 已訂

> 🌿 Milford Sound 前一晚中繼站，建議早入住、早休息以備隔天早出發

**Te Anau Lakefront Backpackers（蒂阿瑙湖畔背包客旅館）**

| 項目 | 內容 |
|------|------|
| Check-in | 5/13（三）14:00–20:00 |
| Check-out | 5/14（四）10:00 前 |
| 房型 | 花園艙間 |
| 付款 | 子擎 Booking.com 排程扣款 · NZD 130（≈ TWD 2,414）· 5/6 自動扣款（末碼 0950）|
| 免費取消 | ✅ 截止 5/7（三）23:59 NZST |

### 皇后鎮 Queenstown（5/14 ~ 5/15，2 晚）✅ 已訂

**The Black Sheep Backpackers（黑綿羊背包客旅舍）**

| 項目 | 內容 |
|------|------|
| Check-in | 5/14（四）15:00–23:30 |
| Check-out | 5/16（六）08:00–10:00 |
| 房型 | 雙人房 |
| 付款 | 子擎 Booking.com 排程扣款 · NZD 313.58（≈ TWD 5,823）· 5/7 自動扣款（末碼 0950）|
| 免費取消 | ✅ 截止 5/8（四）23:59 NZST |

---

## 💰 預算 Budget

### 已確認費用

| 項目 | 訂購者 | 金額（NTD）| 備註 |
|------|--------|-----------|------|
| TPE → SIN（酷航 TR867）| 佩璇 | 9,328 | 每人 20kg |
| SIN → MEL（捷星 JQ008）| 佩璇 | 21,156 | 每人 20kg |
| MEL → CHC（捷星 JQ171）| 佩璇 | 15,694 | 每人 30kg |
| ZQN → AKL → TPE（JQ292 + CI054）| 子擎 | 33,864 | 每人 30kg + 2×23kg |
| **機票合計** | | **80,042** | |
| 租車 Snap Rentals Corolla Wagon 12天 | 子擎 | NZD 891.58（≈ TWD 16,557）| Simply Better Cover · 單程費 $0 |
| 住宿 新加坡 4/29–30 | 子擎 | $5,156 | 諾維維多利亞飯店，trip.com Apple Pay，4/11 已付（不退款）|
| 住宿 墨爾本 5/02–03 | 子擎 | AUD $322.03（≈ $7,239）| 維多利亞酒店，booking.com，4/29 排程扣款（免費取消至 4/30 AEST）|

### 🏨 紐西蘭住宿費用（已訂，4/13 子擎統一訂房）

| 城市 | 飯店 | 晚數 | 金額 | 台幣估算 | 付款時間 | 狀態 |
|------|------|------|------|----------|----------|------|
| 基督城 | Urbanz 烏爾班茲旅舍 | 3 | NZD 345.65 | ≈ 6,418 | 4/30 排程扣款 | ⏳ |
| 葛雷茅斯 | 地球村旅客酒店 | 1 | NZD 99 | ≈ 1,838 | 5/03 排程扣款 | ⏳ |
| 約瑟夫冰河 | Wild Kea Lodge | 1 | NZD 112.50 | ≈ 2,089 | 5/03 排程扣款 | ⏳ |
| 瓦納卡 | Wanaka Bakpaka | 1 | €10.30 + NZD 110.50 | ≈ 2,414 | 4/13 訂金已付 + 到店付 | ✅/⏳ |
| 蒂卡波 | Haka House Hostel | 1 | NZD 337.36 | ≈ 6,264 | 4/14 已付 | ✅ 不退款 |
| 奧馬魯 | 奧馬魯背包客旅館 | 1 | NZD 139.32 | ≈ 2,587 | 5/06 排程扣款 | ⏳ |
| 但尼丁 | Sahara Guest House | 1 | — | ≈ 2,724 | 待扣款 | ⏳ |
| 蒂阿瑙 | Te Anau Lakefront Backpackers | 1 | NZD 130 | ≈ 2,414 | 5/6 排程扣款 | ⏳ |
| 皇后鎮 | The Black Sheep Backpackers | 2 | NZD 313.58 | ≈ 5,823 | 5/7 排程扣款 | ⏳ |
| **NZ 住宿合計** | | **12 晚** | | **≈ TWD 32,571** | | |

### 待估算

| 類別 | 備註 |
|------|------|
| 餐飲 | |
| 活動 / 票券 | 米佛峽灣・冰河直升機・Skyline・跳傘等 |

---

## 💸 帳目紀錄

| 日期 | 轉帳者 | 收款者 | 金額 | 管道 | 說明 | 狀態 |
|------|--------|--------|------|------|------|------|
| 2月頃 | 佩璇 | 易遊網 | $9,328 | 信用卡 | TPE→SIN 雙人 | ✅ |
| 2月頃 | 佩璇 | 易遊網 | $21,156 | 信用卡 | SIN→MEL 雙人 | ✅ |
| 2月頃 | 佩璇 | trip.com | $15,694 | 信用卡 | MEL→CHC 雙人 | ✅ |
| 2月頃 | 子擎 | trip.com | $33,864 | 信用卡 | ZQN→AKL→TPE 雙人 | ✅ |
| 2026/4/8 | 子擎 | Snap Rentals | NZD 891.58（≈ TWD 16,557）| Line Pay 信用卡 | 租車 Corolla Wagon 12天（CHC→ZQN）Simply Better Cover | ✅ |
| 2026/4/11 | 子擎 | 諾維維多利亞飯店（trip.com）| $5,156 | Apple Pay | 新加坡住宿 2晚（不退款）| ✅ 已付款 |
| 2026/4/29 | 子擎 | 維多利亞酒店（booking.com）| AUD $322.03（≈ $7,239）| 中信 Line Pay 末碼 0950 | 墨爾本住宿 2晚 | ⏳ 排程扣款 |
| 2026/4/13 | 子擎 | Urbanz 烏爾班茲旅舍（booking.com）| NZD 345.65（≈ TWD 6,418）| Line Pay 中信信用卡 | 基督城住宿 3晚 | ⏳ 4/30 排程扣款 |
| 2026/4/13 | 子擎 | 地球村旅客酒店（booking.com）| NZD 99（≈ TWD 1,838）| Line Pay 中信信用卡 | 葛雷茅斯住宿 1晚 | ⏳ 5/03 排程扣款 |
| 2026/4/13 | 子擎 | Wild Kea Lodge（booking.com）| NZD 112.50（≈ TWD 2,089）| Line Pay 中信信用卡 | 約瑟夫冰河住宿 1晚 | ⏳ 5/03 排程扣款 |
| 2026/4/13 | 子擎 | Wanaka Bakpaka（Hostelworld）| €10.30 已付 + NZD 110.50 到店付（≈ TWD 2,414）| Line Pay 中信信用卡 | 瓦納卡住宿 1晚 | ✅ 訂金 / ⏳ 到店 |
| 2026/4/13 | 子擎 | Haka House Hostel Tekapo（booking.com）| NZD 337.36（≈ TWD 6,264）| Line Pay 中信信用卡 | 蒂卡波住宿 1晚（不退款）| ✅ 4/14 已付 |
| 2026/4/13 | 子擎 | 奧馬魯背包客旅館（booking.com）| NZD 139.32（≈ TWD 2,587）| Line Pay 中信信用卡 | 奧馬魯住宿 1晚 | ⏳ 5/06 排程扣款 |
| 2026/4/13 | 子擎 | Sahara Guest House（booking.com）| ≈ TWD 2,724 | Line Pay 中信信用卡 | 但尼丁住宿 1晚 | ⏳ 待扣款 |
| 2026/4/14 | 子擎 | Te Anau Lakefront Backpackers（booking.com）| NZD 130（≈ TWD 2,414）| 中信 末碼 0950 | 蒂阿瑙住宿 1晚 | ⏳ 5/6 排程扣款 |
| 2026/4/14 | 子擎 | The Black Sheep Backpackers（booking.com）| NZD 313.58（≈ TWD 5,823）| 中信 末碼 0950 | 皇后鎮住宿 2晚 | ⏳ 5/7 排程扣款 |
| 2026/4/29 | 佩璇 | Silver River Alpine Stargazing（Bookme）| NZD 190（≈ TWD 3,528）| 信用卡 | 蒂卡波觀星 2 位成人（天氣取消全額退款）| ✅ 已付 |

---

## 🎒 行前準備清單 Checklist

### 護照 / 文件
- [ ] 護照有效期確認（6 個月以上）
- [ ] 護照影本備份（雲端 + 紙本）

### 機票 / 簽證
- [ ] 確認所有電子機票
- [ ] 澳洲 ETA 申請
- [ ] 紐西蘭 NZeTA 申請
- [ ] 新加坡免簽確認（30 天）

### 行李
- [ ] 確認各段行李限重（去程 20kg / 段）
- [ ] 行李秤 + 行李鎖（TSA）
- [ ] 頸枕 / 眼罩（SIN→MEL 過夜）

### 租車 / 駕駛
- [x] ~~租車預訂~~ ✅ Snap Rentals Corolla Wagon，NZD 891.58，CHC 取車 5/4，ZQN 還車 5/16 09:00
- [x] ~~還車地點確認~~ ✅ Snap Rentals ZQN Airport，5/16 09:00
- [ ] 國際駕照（2 人均需辦，至監理所）
- [ ] 下載 Google Maps 紐西蘭離線地圖（山區無訊號）

### 金融 / 通訊
- [x] ~~新加坡 / 澳洲 / 紐西蘭 SIM 卡 / eSIM~~ ✅ 全程 eSIM：NZ One NZ #26KK234780701（含通話・主力）+ Spark VSR087784（備用）+ AUS Optus + SIN Singtel（詳見下方通訊資訊）
- [ ] 準備 AUD / NZD / SGD 現金
- [ ] 通知銀行出國

### 其他
- [ ] 電源轉接頭（I 型，澳紐通用）
- [x] ~~旅遊保險（含醫療・取消・行李）~~ ✅ 兆豐産險 2915CTAY614046 + 富邦旅平險 3226CT20D09794（詳見下方通訊 & 保險）

---

## ⚠️ 重要備註 Important Notes

### 行李

| 航段 | 託運額度 | 備註 |
|------|----------|------|
| TPE → SIN（TR867）| 雙人共 20kg | 超重費貴 |
| SIN → MEL（JQ008）| 雙人共 20kg | 注意重量 |
| MEL → CHC（JQ171）| 雙人共 30kg | 比去程多 10kg |
| ZQN → AKL（JQ292）| 每人 30kg | |
| AKL → TPE（CI054）| 每人 **2×23kg**（共 4 件）| 行李直掛，BNE 無需重辦 |

### 5/16 離境時間線

```
07:00   起床準備
09:00   Snap Rentals ZQN 還車（加滿油）
09:30   機場報到（建議 09:30 前）
10:50   JQ292 ZQN 起飛
12:40   抵達 AKL D 廳 → 移至 I 廳
19:30   CI054 AKL 起飛 → 途經 BNE → TPE
05:40+1 抵達桃園
```

---

## 📋 紐西蘭待辦清單 NZ To-Do

### 🏨 住宿
- [x] ~~🔥 皇后鎮 2 晚（5/14–5/15）~~ ✅ The Black Sheep Backpackers
- [x] ~~🔥 蒂阿瑙 Te Anau 1 晚（5/13）~~ ✅ Te Anau Lakefront Backpackers
- [x] ~~⭐⭐ 蒂卡波 2 晚（5/10–5/11）~~ ✅ Haka House Hostel Tekapo（1晚）
- [x] ~~基督城 3 晚（5/04–5/06）~~ ✅ Urbanz 烏爾班茲旅舍
- [x] ~~葛雷茅斯 1 晚（5/07）~~ ✅ 地球村旅客酒店 Global Village
- [x] ~~約瑟夫冰河 1 晚（5/08）~~ ✅ Wild Kea Lodge Franz Josef
- [x] ~~瓦納卡 1 晚（5/09）~~ ✅ Wanaka Bakpaka
- [x] ~~奧馬魯 1 晚（5/11）~~ ✅ 奧馬魯背包客旅館（新增站）
- [x] ~~但尼丁 1 晚（5/12）~~ ✅ Sahara Guest House（行程調整恢復，確認保留）
- [x] ~~墨爾本 2 晚（5/02–5/03）~~ ✅ 維多利亞酒店
- [x] ~~新加坡 2 晚（4/29–4/30）~~ ✅ 諾維維多利亞飯店

### 🎡 活動預訂
- [x] ~~⛵ Milford Sound 遊船（NZ D11）~~ ✅ GreatSights 大巴一日遊（NT$ 6,730，Klook）
- [x] ~~🚁 冰河直升機：Franz Josef 或 Fox Glacier（NZ D5，擇一）~~ ✅ **法蘭士約瑟夫冰川直升機健行（NZ D6 07:30）** Klook · HAU427597 · 憑證 KLK0703585975
- [x] ~~🌌 Mt John 觀星（NZ D7，蒂卡波）~~ ✅ Alpine Stargazing Experience · Silver River · XYSA26X5711441（Bookme）
- [ ] 🚠 Skyline Gondola 皇后鎮（NZ D12）
- [ ] 🪂 跳傘 / 噴射船（NZ D12，擇一）
- [ ] 🍽 Tramway Restaurant 電車晚餐，NZD 149（NZ D1 或 D3）
- [ ] 🎟 Christchurch Pass，NZD 114（NZ D3）
- [ ] 🐬 阿卡羅阿自然之旅，NZD 125（NZ D2）

### 🚗 租車 / 出行
- [ ] ⚡ **出發前電話聯繫 Snap Rentals（0800 288 699）**，告知 5/4 預計抵達時間（約 18:00），確認非營業時間取車安排
- [ ] 國際駕照確認（2 人均有效）
- [ ] 下載 Google Maps 紐西蘭離線地圖（山區無訊號）

### 🛂 簽證
- [ ] 澳洲 ETA
- [ ] 紐西蘭 NZeTA

---

## 📱 通訊 & 保險資訊

### 📡 網卡總覽

| 地區 | 電信商 | 有無通話 | 流量 | 天數 | 角色 | 平台 | 訂單號 | 費用 |
|------|--------|----------|------|------|------|------|--------|------|
| 🇳🇿 紐西蘭 | **One NZ（Vodafone）** | ✅ 含通話 | 10 GB | 60 天 | **主力** | KKday | **26KK234780701** | NT$1,100 |
| 🇳🇿 紐西蘭 | Spark 5G | ❌ 數據 | 20 GB | 15 天 | 備用 | Klook | **VSR087784** | NT$575 |
| 🇦🇺 澳洲 | Optus 5G | ❌ 數據 | 5 GB | 30 天 | 主力 | Klook | — | NT$132 |
| 🇸🇬 新加坡 | Singtel | ❌ 數據 | 每日不限 | 3 天 | 主力 | Klook | — | NT$107 |

📄 憑證：[One NZ](<../docs/assets/Kkday-NZ-OneNZ通話-10GB-60days.pdf>) · [Spark](<../docs/assets/Klook voucher-NZ-Spark5G-20GB-15days.pdf>) · [Optus](<../docs/assets/Klook voucher-MEL-Optus5G-5GB-30days.pdf>) · [Singtel](<../docs/assets/Klook voucher-SIN-Singtel5G-不限-3days.pdf>)

---

### 📌 使用策略

| 地區 / 期間 | 使用哪張 | 說明 |
|-------------|----------|------|
| 🇸🇬 新加坡（4/29–5/1）| Singtel | 3 天每日不限，啟用日 4/29 |
| 🇦🇺 澳洲（5/1–5/4）| Optus 5G | 5 GB / 30 天，啟用日 5/1 |
| 🇳🇿 紐西蘭（5/4–5/16）| **One NZ（主）** + Spark（備）| One NZ 有通話；Spark 山區備援 |

- **One NZ 主力原因**：有語音通話，可直接打租車公司 / 住宿電話
- **Spark 備援**：覆蓋互補，可設為次 SIM 備用數據
- **iPhone 建議**：One NZ 設主 SIM（接打電話）；Spark 設次 SIM（僅數據）
- **熱點分享**：⚠️ 是否支援各家未確認，請查 APN 設定

---

### 📶 實用提醒

- ⛰ **山區無訊號**：Haast Pass、Fiordland、Arthur's Pass 等地可能完全無訊號，出發前下載 Google Maps 離線地圖
- 📲 **出發前安裝**：eSIM 安裝需要 Wi-Fi，建議台灣出發前完成，到各地後再啟用
- 🔄 **啟用時機**：設定 → 行動服務 → 手動啟用對應 eSIM（同一時間只啟用當地那張）
- 🗺 **離線地圖**：推薦 Google Maps 預先下載南島各區，或使用 Maps.me

---

### 🆘 無訊號時怎麼做

1. 開啟飛航模式 → 等 10 秒 → 關閉
2. 設定 → 行動服務 → 確認對的 eSIM 已啟用
3. 手動選網：設定 → 行動服務 → 選 SIM → 網路選擇 → 手動掃描
4. 重新開機
5. One NZ 無訊號 → 切換至 Spark eSIM

---

### ⚠️ eSIM 待確認

- One NZ 60 天有效期：從購買日還是啟用日起算
- 兩張 NZ eSIM 是否可同時啟用（雙 eSIM 模式）
- Spark 是否支援熱點分享
- APN 設定（One NZ / Spark / Optus）

---

### 🛡 保險資訊

**🚨 出事時查這裡**

| 情境 | 找哪張保單 | 馬上要做什麼 |
|------|------------|-------------|
| ✈️ 班機延誤 | 兆豐 + 富邦 | 向航空公司索取**延誤證明**（蓋章）・保留所有收據 |
| 🧳 行李不見 | 兆豐 + 富邦 | 機場拿 **PIR** → 先申延誤 → 21天後申遺失 |
| 🏥 海外就醫 | 兆豐（主）+ 富邦 | 直接就醫 → 打 **0800-053-588** → 留收據正本 + 診斷書 |
| 🚗 租車擦撞 | 富邦租車附加 | 保留扣款收據 + 合約，補自負額費用 |

👉 詳細流程、不賠項目、理賠文件：[insurance-summary.md](insurance-summary.md)

---

#### 兆豐產險 旅遊綜合保險（大洋洲）

| 項目 | 內容 |
|------|------|
| 保單號碼 | **2915CTAY614046** |
| 保障期間 | 2026/04/29 ~ 2026/05/17 |
| 保障地區 | 紐西蘭 + 澳洲 |
| 保費 | **TWD 1,428** |
| 緊急聯絡電話 | **0800-053-588** |
| 保單 PDF | [兆豐保單-SIN_MEL_NZ-旅平.pdf](../docs/assets/兆豐保單-SIN_MEL_NZ-旅平.pdf) |

**可申請情境：** 班機延誤 · 行李遺失 · 海外就醫（急診）· 緊急撤離

#### 富邦旅平險（大洋洲 + 新加坡）

| 項目 | 內容 |
|------|------|
| 主約保單號 | **3226CT20D09794**（計畫三 / 組合15，500萬，TWD 2,466）|
| 附加保單號 | **3226CT20D09795**（租車加購保障，TWD 63）|
| 保障期間 | 2026/04/29 13:00 ~ 2026/05/17 13:00 |
| **總保費** | **TWD 2,529** |

**可申請情境：** 班機取消 / 延誤 · 行李延誤 · 醫療費用 · 租車碰撞（附加）

| 保單 | 連結 |
|------|------|
| 旅平保單 | [富邦保單-SIN_MEL_NZ-旅平.pdf](../docs/assets/富邦保單-SIN_MEL_NZ-旅平.pdf) |
| 租車保單 | [富邦保單-SIN_MEL_NZ-租車.pdf](../docs/assets/富邦保單-SIN_MEL_NZ-租車.pdf) |

> 💡 兩張保單合計：TWD 1,428（兆豐）+ TWD 2,529（富邦）= **TWD 3,957**

---

### 🎟 活動憑證 PDF

| 活動 | 憑證 |
|------|------|
| ⛵ Milford Sound GreatSights 遊船 | [Klook-NZ-Milford-GreatSight.pdf](<../docs/assets/Klook voucher-NZ-Milford-GreatSight.pdf>) |
| 🌌 蒂卡波觀星 Alpine Stargazing（Silver River）| [佩璇訂蒂卡波觀星bookme.pdf](<../docs/assets/佩璇訂蒂卡波觀星bookme.pdf>) |
